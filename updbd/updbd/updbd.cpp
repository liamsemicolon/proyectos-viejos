// updbd.cpp : Defines the entry point for the console application.
//
...
public:
	CBlockDevice(const char *sFileName) : _read_only(false) {
		...
		_fsize = _lseeki64(_fp, 0, SEEK_END);
	...
	void seek(uint32_t sector) {
		__int64 offset = (__int64)sector * 512;
		//printf("seek %d * 512 = %ld\n", sector, offset);
		_lseeki64(_fp, offset, SEEK_SET);
	}
...
public:
	CUDPBDServer(class CBlockDevice &bd) : _bd(bd), _block_shift(0) {
		...
		setsockopt(s, SOL_SOCKET, SO_BROADCAST, (const char*)&broadcastEnable, sizeof(broadcastEnable));
	}
...
private:
	...
	void handle_cmd_info(struct sockaddr_in &si_other, struct SUDPBDv2_InfoRequest *request) {
		...
		// Send packet to ps2
		if (sendto(s, (const char *)&reply, sizeof(reply), 0, (struct sockaddr*) &si_other, sizeof(si_other)) == -1) {
	...
	void handle_cmd_read(struct sockaddr_in &si_other, struct SUDPBDv2_RWRequest *request) {
		...
		while (blocks_left > 0) {
			...
			// Send packet to ps2
			if (sendto(s, (const char *)&reply, sizeof(struct SUDPBDv2_Header) + 4 + (reply.bt.block_count * _block_size), 0, (struct sockaddr*) &si_other, sizeof(si_other)) == -1) {
				throw runtime_error("sendto");
			}
			reply.hdr.cmdpkt++;
		}
